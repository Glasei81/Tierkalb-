...
Ausgabe:
  - Täglich 6:00 Uhr: Geburten + Trächtigkeitskontrolle + fällige Impfungen
...
def build_status_message(farm_id: str, farm_name: str) -> str:
    ...
    if brunft_faellig:
        ...
        for b in brunft_faellig:
            ...
            if tage <= 0:
                ...
                bis = (heute + timedelta(days=2)).strftime("%d.%m.")
                zeile += f"\n    ↳ Besamungsfenster bis ca. {bis} — Besamer anrufen!"
            elif tage <= 3:
    ...
    if impfungen_faellig:
        ...
        for imp in impfungen_faellig:
            em   = imp.get("emoji", "🐄")
            name = imp["name"]
            letzte = imp.get("letzte_impfung")
            if letzte:
                tage = int(imp.get("tage_seit_impfung") or 0)
                lines.append(f"  💊 {em} {name} — letzte Impfung vor {tage} Tagen")
            else:
        ...
        for farm in db.get_all_farms():
            ...
            if impfungen:
                ...
                for imp in impfungen:
                    ...
                    if letzte:
                        tage = int(imp.get("tage_seit_impfung") or 0)
                        lines.append(f"  {em} {imp['name']} — letzte Impfung vor {tage} Tagen")
                    else:
                        lines.append(f"  {em} {imp['name']} — noch nie geimpft")
